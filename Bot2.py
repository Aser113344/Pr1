...
# --- إعدادات xShellz ---
XSHELLZ_USERNAME = "THAGDSA"
XSHELLZ_PASSWORD = "Aser119936"
XSHELLZ_INTERVAL = 300  # 5 دقائق = 300 ثانية
...
# =====================================================
# === سكربت xShellz Keep Shell ===
# =====================================================

class XShellzKeepShell:
    def __init__(self, username, password):
        self.username = username
        self.password = password
        self.base_url = "https://www.xshellz.com"
    ...
    def login(self):
        try:
            ...
            if response.status_code == 200:
                ...
                if 'u' in cookies and 'pw' in cookies:
                    return True
            return False
        except:
            return False
    ...
    def get_shell_id(self):
        try:
            xpanel_url = f"{self.base_url}/xpanel"
            response = self.session.get(xpanel_url)
            response.raise_for_status()
            match = re.search(r'/xpanel/shell/(\d+)', response.text)
            if match:
    ...
    def keep_shell(self, shell_id):
        try:
            shell_url = f"{self.base_url}/xpanel/shell/{shell_id}"
            response = self.session.get(shell_url)
            ...
            keep_url = f"{self.base_url}/ajax/shell/keep"
            headers = {
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
                'X-Requested-With': 'XMLHttpRequest',
                'X-CSRF-TOKEN': self.csrf_token,
            ...
            response = self.session.post(keep_url, data=data, headers=headers)
    ...
    def run(self):
        ...
        try:
            if not self.login():
                print("[xShellz] فشل تسجيل الدخول")
                return False
            
            shell_id = self.get_shell_id()
            if not shell_id:
                print("[xShellz] لم يتم العثور على شل")
                return False
            
            if not self.keep_shell(shell_id):
                print("[xShellz] فشل تمديد الشل")
                return False
            
            print(f"[xShellz] تم تمديد الشل #{shell_id} بنجاح!")
            return True
        except Exception as e:
            print(f"[xShellz] خطأ: {e}")
            return False


async def xshellz_background_task():
    """مهمة خلفية لتشغيل xShellz كل 5 دقائق (باستخدام asyncio)"""
    print("[xShellz] بدء المهمة الخلفية...")
    while True:
        try:
            # تشغيل الكود المتزامن في executor
            loop = asyncio.get_event_loop()
            tool = XShellzKeepShell(XSHELLZ_USERNAME, XSHELLZ_PASSWORD)
            await loop.run_in_executor(None, tool.run)
        except Exception as e:
            print(f"[xShellz] خطأ في المهمة الخلفية: {e}")
        
        # انتظار 5 دقائق
        await asyncio.sleep(XSHELLZ_INTERVAL)
...
def get_admin_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("🐚 حالة xShellz", callback_data='xshellz_status')],
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- زر حالة xShellz ---
    elif data_str == 'xshellz_status':
        await safe_edit("🐚 جاري تشغيل xShellz...", reply_markup=get_admin_keyboard())
        try:
            loop = asyncio.get_event_loop()
            tool = XShellzKeepShell(XSHELLZ_USERNAME, XSHELLZ_PASSWORD)
            success = await loop.run_in_executor(None, tool.run)
            if success:
                await safe_edit("🐚 xShellz:\n\n✅ تم تمديد الشل بنجاح!\n⏱️ يعمل كل 5 دقائق تلقائياً", reply_markup=get_admin_keyboard())
            else:
                await safe_edit("🐚 xShellz:\n\n❌ فشل في تمديد الشل", reply_markup=get_admin_keyboard())
        except Exception as e:
            await safe_edit(f"🐚 xShellz:\n\n❌ خطأ: {e}", reply_markup=get_admin_keyboard())
...
async def post_init(application: Application) -> None:
    """يتم تشغيلها بعد بدء البوت - لبدء مهمة xShellz"""
    asyncio.create_task(xshellz_background_task())
    print("[xShellz] تم بدء المهمة الخلفية (كل 5 دقائق)")


def main() -> None:
    # بدء البوت مع مهمة xShellz
