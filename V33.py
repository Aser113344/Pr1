...
API_GEMINI = 'AIzaSyA0_MDjHA9CriP14-OZW2aYAhmJ3KeY68M'  # مفتاح API الخاص بـ GMINI

bot_creator = "@N1_ORGANIZATION_1"   # تم التحديث لقناتك الجديدة 
...
#################### Keep-Alive - إبقاء البوت شغال ############

def keep_alive():
    """دالة لإرسال request لـ Telegram API كل 20 ثانية للحفاظ على تشغيل البوت"""
    while True:
        try:
            # إرسال طلب getMe لـ Telegram API للحفاظ على الاتصال
            bot.get_me()
            logging.info("Keep-alive ping sent to Telegram API - Bot is active")
        except Exception as e:
            logging.warning(f"Keep-alive ping failed: {e}")
        time.sleep(20)  # انتظار 20 ثانية قبل الريكويست التالي

# تشغيل keep_alive في thread منفصل
keep_alive_thread = Thread(target=keep_alive, daemon=True)
keep_alive_thread.start()
logging.info("تم تشغيل Keep-Alive - سيتم إرسال ping كل 20 ثانية")
...
def start(message):
    ...
    if message.from_user.username in banned_users:
        bot.send_message(message.chat.id, "تم حظرك من البوت. تواصل مع المطور @N1_HUMEN")
        return
...
ADMIN_CHANNEL = '@N1_ORGANIZATION_1'
...
def instructions(message):
    if message.from_user.username in banned_users:
        bot.send_message(message.chat.id, "تم حظرك من البوت تواصل مع المطور @ii3IllaX")
        return
...
def handle_file(message):
    ...
    try:
        ...
        # تم إزالة فحص الفيروسات بناءً على طلب المستخدم
        pass
